...
export class TelemetryService extends ComponentStore<Location> {
  ...
  readonly watchPosition = this.effect(trigger$ => trigger$.pipe(
    switchMap(() => {
      return BackgroundGeolocation.addWatcher(
        ...
        (location, error) => {
          ...
          this.ngZone.run(async () => {
            ...
            console.log(new Date(location.time).toISOString());
            this.setState({
              ...location
            });
          });
    ...
    tap({
      ...
      finalize: () => { // this is tap() (rxjs) specific, not just componentStore. finalize() gets called when 
